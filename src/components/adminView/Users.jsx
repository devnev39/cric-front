...
function Users() {
  ...
  const fetchUsers = async () => {
    const resp = await (
      await fetch(`${settings.BaseUrl}/users`, {credentials: 'include'})
    ).json();
    if (resp.status === 200) {
      setUsers(resp.data);
    } else {
        ...
        >
          ...
          >
            <input
              ...
              onChange={(e) => updateUser(user._id)}
            />
          </div>
  ...
  const deleteUser = async (userId) => {
    ...
    const resp = await (
      await fetch(`${settings.BaseUrl}/users/${userId}`, {
        method: 'DELETE',
        credentials: 'include',
      })
    ).json();
