...
const AuctionConfig = () => {
    ...
    const fetchAuctions = async () => {
        const resp = await (await fetch(`${settings.BaseUrl}/admin/auctions`,{credentials : "include"})).json()
        if (resp.status === 200){
            ...
            return (
                <div key={`${user._id}}`} className="row mb-4 shadow-sm rounded h5 auctionRowContent p-2">
                    ...
                    <div className="col-1 d-flex justify-content-center" style={{"borderRight" : `2px dotted`}}>
                        <input type="checkbox" checked={user.AllowLogin} onChange={(e) => updateUser(user)} />
                    </div>
                    <div className="col-2 d-flex justify-content-center">
                        <button className="btn btn-danger btn-sm mx-2" onClick={() => deleteUser(user)}><i className="fa-solid fa-trash-can"></i></button>
    ...
    const deleteUser = async (user) => {
        if (! window.confirm("Do you want do delete this auction ? All of the data will be deleted !")) {
          return;
        }
        const resp = await (await fetch(`${settings.BaseUrl}/admin/auctions/${user._id}`,{
            method : "DELETE",
            credentials : "include",
            headers : {
    ...
    return (
        ...
        <div className="closeFormDisplay closeForm" id="submitForm">
            <div className="d-flex justify-content-center">
                <SubmitForm postUrl={"/admin/auctions"} modelKey={"AuctionViewModelAdmin"} neglects={["_id"]} closeFunc = {() => toggleSubmitForm("submitForm")} parentKey="submit1" />
            </div>
        </div>
        <div className="closeFormDisplay closeForm" id="updateForm">
            <div className="d-flex justify-content-center">
                {currentAuction ? <UpdateForm modelKey = {"AuctionViewModelAdmin"} neglects = {["_id"]} model = {currentAuction} postUrl={`/admin/auctions/${currentAuction._id}`} closeFunc = {() => toggleSubmitForm("updateForm")} parentKey="update1" setFunc={setAuction} navigate = {setTrigger} /> : null}
            </div>
        </div>
        </>
    )
