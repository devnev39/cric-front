...
function BidConsole(props) {
    ...
    const placeBid = async () => {
        ...
        if(!bid){alert("Bad input !");return;}
        if(!selectedTeam){alert("Select a team !");return;}
        if(selectedTeam.Current < bid){alert("No budget to buy the player !");return;}
        const bidObj = {
            amt : bid,
            player : props.player,
            team : selectedTeam
                    ...
                    <div className="bidControlDiv mt-5" id="bidControlDiv">
                        <div className="row">
                            ...
                            <div className="col-6 d-flex justify-content-center">
                                <input type="number" className="" placeholder="Bid Amt." id="bidAmountInput"></input>
                            </div>
                        </div>
                        <div className="d-flex justify-content-center mt-4">
                            <button className="btn btn-success mx-5" onClick={() => placeBid()}>Bid</button>
                            <button className="btn btn-danger mx-5" onClick={() => props.closeFunc()}>Close</button>
                        </div>
                    </div>
                </div>
                 : null}
