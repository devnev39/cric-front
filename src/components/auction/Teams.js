...
function Teams(props) {
    ...
    return (
        ...
        <div className="closeFormDisplay closeForm" id="updateForm">
            <div className="d-flex justify-content-center">
                {currentTeam ? <UpdateForm modelKey = {"team"} neglects = {[]} model = {currentTeam} postUrl={`/auction/${props.auctionObj._id}/teams/${currentTeam._id}`} closeFunc = {() => toggleSubmitForm("updateForm")} parentKey="update1" navigate={props.trigger} /> : null}
            </div>
        </div>
        </>
    )
