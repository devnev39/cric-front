...
function Option(props) {
    ...
    const showRuleConsole = () => {
        ...
        if(e.classList.contains("display")){
            ...
            document.getElementById("optionMainDiv").classList.toggle("blurBackground");
        }else{
            e.classList.toggle("display");
            e.classList.toggle("closeForm");
            e.classList.toggle("openFormDisplay")
    ...
    const addNewRule = async  () => {
        ...
        if(ruleName === ""){alert("Rule name empty !"); return;}
        let rule = document.getElementById("ruleInput").value;
        const type = document.getElementById("wimodelSelection").value;
        const rule_check = JSON.parse(JSON.stringify(rule));
        if(rule === ""){alert("Empty rule !"); return;}
            ...
            <div className="RulesContainerRoot mt-5 d-flex justify-content-center" key={"2"}>
                <div className="shadow rounded py-3 px-5 w-100">
                        ...
                        <div className="h5 w-100">
                            {
                                ...
                                props.auctionObj.Rules.map(rule => {
                                    return (
                                        <>
                                        <div className="ruleDiv shadow rounded p-3 m-3 w-100" key={rule.ruleName}>
                                            <div className="row">
                                                <div className="col-2">
                                                    {rule.ruleName}
                                                </div>
                    ...
                    <div className="d-flex justify-content-center">
                            <button className="btn btn-success" onClick={() => showRuleConsole()}>Add Rule</button>
                        </div>
                </div>
            </div>
        </div>
         <div className="closeFormDisplay closeForm" style={{"top" : "30%", "left" : "40%"}} id="ruleConsoleForm">
         <div className="d-flex justify-content-center">
             <div className="w-50 p-5 rounded shadow" id="ruleConsole" style={{"backgroundColor" : "rgba(255,255,255,0.9)"}}>
                 <div className="row my-4">
                     <div className="col-4 d-flex justify-content-center font-weight-bold">
                         <label>Name : </label>
                     </div>
                     <div className="col-8 d-flex justify-content-center">
                         <input id="ruleName" type="text" className="w-100" placeholder="rule name"/>
                     </div>
                 </div>
                 <div className="row my-4">
                     <div className="col-4 d-flex justify-content-center font-weight-bold">
                     ...
                     <div className="col-8 d-flex justify-content-center">
                         ...
                         <div className="wiModels border border-primary mx-5 p-3">
                            {
                                wiModel ? 
                                Object.keys(wiModel).map((key) => {return (<><strong key={key}>{`${key}`}</strong><br /></>)})
                                : null
                            }
                         </div>
                     </div>
                 ...
                 <div className="row my-4">
                     ...
                     <div className="col-8 d-flex justify-content-center">
                         <input id="ruleInput" className="w-100" type="text" placeholder="rule - eg. (AuctionedPrice - SoldPrice) / AuctionedPrice"/>
                     </div>
                 </div>
                 <div className="d-flex justify-content-center">
                    <button className="btn btn-success mr-5" onClick={() => addNewRule()}>Add</button>
                    <button className="btn btn-danger" onClick={() => showRuleConsole()}>Close</button>
                 </div>
             </div>
         </div>
     </div>
