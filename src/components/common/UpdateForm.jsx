...
function UpdateForm(props) {
  ...
  useEffect(() => {
    for (const key of Object.keys(modelJson)) {
      ...
      document
          .getElementById(`input-${key}-${props.parentKey}`)
            ...
            setState((model) => {
              ...
              document.getElementById(`input-${key}-${props.parentKey}`).value
                ...
                if (modelJson[key] === 'number') {
                  model[key] = +document.getElementById(
                      `input-${key}-${props.parentKey}`,
                  ).value;
                } else if (modelJson[key] === 'checkbox') {
                  model[key] = document.getElementById(
                      `input-${key}-${props.parentKey}`,
                  ).checked;
                } else {
  ...
  useEffect(() => {
    for (const key of Object.keys(modelJson)) {
      ...
      document.getElementById(`input-${key}-${props.parentKey}`).disabled =
        !updateState;
      if (modelJson[key] === 'checkbox') {
        document.getElementById(`input-${key}-${props.parentKey}`).checked =
          props.model[key];
      } else {
        document.getElementById(`input-${key}-${props.parentKey}`).value =
          props.model[key];
    ...
    >
      ...
      <div className="d-flex justify-content-center mt-5">
        ...
        <button
          ...
          id="saveBtn"
          onClick={putModelRequest}
        >
          Save
        </button>
        {props.closeFunc ? (
          <button className="btn btn-danger mx-5" onClick={props.closeFunc}>
            Close
          </button>
        ) : null}
      </div>
