...
const Auctions = () => {
  ...
  const createAuctionStack = () => {
    return auctions.map((auction) => {
      return (
        <div key={`${auction._id}}`} className="row mb-4 shadow-sm rounded h5 auctionRowContent" onClick={() => {
          navigate(`/auction/${auction._id}`, {state: {auction: auction}});
        }}>
  ...
  const getAuctions = async () => {
    ...
    if (response.status === 601) {
      navigate(`/auction/${response.data._id}`, {state: {auction: response.data}}); return;
    }
